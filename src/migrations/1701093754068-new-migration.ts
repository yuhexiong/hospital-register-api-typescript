...
export class NewMigration1701093754068 implements MigrationInterface {
  name = 'NewMigration1701093754068'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE TABLE \`doctor\` (\`id\` varchar(20) NOT NULL COMMENT '醫師工號', \`name\` varchar(20) NOT NULL COMMENT '姓名', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    await queryRunner.query(`CREATE TABLE \`reservationOverview\` (\`id\` bigint NOT NULL AUTO_INCREMENT COMMENT '流水號', \`date\` varchar(20) NOT NULL COMMENT '日期, YYYY-MM-DD', \`timeSlot\` enum ('morning', 'afternoon', 'evening') NOT NULL COMMENT '時段', \`division\` enum ('00', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12', '13', '14', '15', '22', '40', '60', '81', '82', '83', '84', '2A', '2B') NOT NULL COMMENT '科別' DEFAULT '00', \`doctorId\` varchar(20) NOT NULL COMMENT '醫師工號', \`canReverse\` tinyint NOT NULL COMMENT '是否開放預約' DEFAULT 1, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    await queryRunner.query(`CREATE TABLE \`reservationDetail\` (\`id\` bigint NOT NULL AUTO_INCREMENT COMMENT '流水號', \`reverseTime\` datetime NULL COMMENT '預約時間', \`reservationOverviewId\` bigint NOT NULL COMMENT '預約總表id', \`no\` bigint NOT NULL COMMENT '看診號', \`type\` enum ('onSite', 'reverse', 'all', 'not') NOT NULL COMMENT '給號類別', \`patientId\` bigint NULL COMMENT '病人id', \`note\` varchar(200) NULL COMMENT '備註', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    await queryRunner.query(`CREATE TABLE \`clinic\` (\`id\` bigint NOT NULL AUTO_INCREMENT COMMENT '流水號', \`reservationDetailId\` bigint NOT NULL COMMENT '預約細項id', \`date\` varchar(20) NOT NULL COMMENT '門診日期, YYYY-MM-DD', \`beginTime\` datetime NULL COMMENT '開始看診時間', \`finishTime\` datetime NULL COMMENT '完診時間', \`isNhi\` tinyint NOT NULL COMMENT '是否掛健保, true=健保, false=自費' DEFAULT 0, \`hasIcCard\` tinyint NOT NULL COMMENT '是否有健保卡' DEFAULT 0, \`cardExceptionCode\` enum ('A000', 'A010', 'A020', 'A030', 'B000', 'C000', 'D000', 'D010', 'E000', 'F000', 'Z000') NULL COMMENT '卡片異常代碼', \`medicalType\` enum ('01', '02', '03', '04', '05', '06', '07', '08', 'AA', 'AB', 'AC', 'AD', 'AE', 'AF', 'AG', 'AH', 'AI', 'BA', 'BB', 'BC', 'BD', 'BE', 'BF', 'CA', 'DA', 'DB', 'DC', 'ZA', 'ZB', 'YC', 'YD', 'YA', 'YE', 'XA', 'YB', 'YF', 'YG', 'YH') NULL COMMENT '就醫類別', \`medicalNumber\` varchar(20) NULL COMMENT '就醫序號', \`caseType\` enum ('01', '02', '03', '04', '06', '08', '09', 'A3', 'B6', 'B7', 'D1', 'D2') NULL COMMENT '案件別' DEFAULT '09', \`doctorId\` varchar(20) NOT NULL COMMENT '醫師工號', \`patientId\` bigint NOT NULL COMMENT '病人id', \`soap\` varchar(300) NULL COMMENT 'soap', \`status\` enum ('REGISTER', 'CLINIC', 'COMPLETE', 'PAID', 'UNREGISTER') NOT NULL COMMENT '門診狀態' DEFAULT 'REGISTER', \`payableAmount\` bigint NOT NULL COMMENT '應繳金額' DEFAULT '0', \`paidAmount\` bigint NOT NULL COMMENT '已繳金額' DEFAULT '0', UNIQUE INDEX \`REL_5acbe4779e33a47784323beffb\` (\`reservationDetailId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
